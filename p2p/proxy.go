...
// describe a proxy decorator to protect a peer networking functionalities
// So that means both a server and a client capabilities should be injected
type PeerProxy interface {
	InjectTransport(server Server) error
}
	...
	go func() {
		...
		for ev := range eventCh {
			func() {
				...
				ctx, _ := context.WithDeadline(context.Background(), time.Now().Add(1*time.Second)) //generally smaller timeout than default (message can be lost)
